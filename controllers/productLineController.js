...
const createProductLine = asyncHandler(async (req, res) => {
    ...
    if (existingProductLine) {
        res.status(400).json({ message: "A product line with this name already exists" });
        return;
    }
...
const deleteProductLine = asyncHandler(async (req, res) => {
    ...
    if (products.length > 0) {
        res.status(400).json({ message: "Product line cannot be deleted because it has associated products" });
        return;
    }
